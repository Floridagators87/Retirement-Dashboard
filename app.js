const STORAGE_KEY='retirementHQ.v6';
const OLD_STORAGE_KEYS=['retirementHQ.v5','retirementHQ.v4','retirementHQ.v3','retirementHQ.v2'];
...
const defaults={careerStart:'2007-10-29',birthDate:'1987-08-17',retirementDate:'2031-09-23',plannedSeparationDate:'2031-09-23',pensionSalary:168017,currentSalaryStart:'2025-10-29',priorSalary:157000,priorSalaryStart:'2024-10-29',annualBudget:100000,investmentGoal:1000000,inflationRate:2.5,expectedReturn:5.5,volatility:12,pensionCola:0,retirementHorizon:40,annualContributions:12000,dropYears:5,dropReturn:5,homeValue:700000,mortgage:277902,mortgageOriginalInput:323000,mortgageInterest:2.75,mortgageStart:'2020-06',mortgageTerm:30,mortgagePI:1398,helocLimit:240000,helocRate:9,otherAssets:0,otherDebt:0,account175:170411,kristaRoth:31000,stevenRoth:48450,baptist403b:10450,city457b:255000,robinhoodRoth:10200,robinhoodTaxable:13500,balancesUpdatedAt:new Date().toISOString(),snapshots:[]};
const settingIds=['careerStart','birthDate','retirementDate','plannedSeparationDate','pensionSalary','currentSalaryStart','priorSalary','priorSalaryStart','annualBudget','investmentGoal','inflationRate','expectedReturn','volatility','pensionCola','retirementHorizon','annualContributions','dropYears','dropReturn','homeValue','mortgage','mortgageOriginalInput','mortgageInterest','mortgageStart','mortgageTerm','mortgagePI','helocLimit','helocRate','otherAssets','otherDebt'];
...
function rule68Date(birth,hire){
  const raw=new Date((birth.getTime()+hire.getTime())/2 + 34*YEAR_DAYS*DAY);
  return new Date(raw.getFullYear(),raw.getMonth(),raw.getDate()+1);
}
function pensionStartForSeparation(separation,eligibility){
  if(separation>=eligibility)return new Date(separation);
  return addMs(eligibility,eligibility-separation);
}
function bridgeAccessibleTotal(d){return Number(d.city457b||0)+Number(d.robinhoodTaxable||0)}
...
function survivabilityModel(d,separationDate,pensionStartDate,separationPension,runs=1800){
  const today=new Date(), yearsToSeparation=Math.max(0,(separationDate-today)/DAY/YEAR_DAYS);
  const bridgeYears=Math.max(0,(pensionStartDate-separationDate)/DAY/YEAR_DAYS);
  const horizon=Math.max(10,Number(d.retirementHorizon));
  const mean=Number(d.expectedReturn)/100,vol=Number(d.volatility)/100,infl=Number(d.inflationRate)/100,cola=Number(d.pensionCola)/100,contrib=Number(d.annualContributions||0);
  const totalNow=investmentTotal(d), bridgeNow=bridgeAccessibleTotal(d), restrictedNow=Math.max(0,totalNow-bridgeNow);
  const helocLimit=Number(d.helocLimit||0),helocRate=Number(d.helocRate||0)/100;
  let bridgeSuccess=0,lifetimeSuccess=0,helocUsedTotal=0;
  for(let run=0;run<runs;run++){
    let bridge=bridgeNow,restricted=restrictedNow,spend=Number(d.annualBudget),heloc=0,bridgeOk=true;
    const workYears=Math.ceil(yearsToSeparation);
    for(let y=0;y<workYears;y++){
      const fraction=(y===workYears-1 && yearsToSeparation%1)?yearsToSeparation%1:1;
      const growth=Math.max(-.95,mean+vol*normalRandom());
      bridge=Math.max(0,bridge*(1+growth*fraction)+contrib*fraction);
      restricted=Math.max(0,restricted*(1+growth*fraction));
      spend*=Math.pow(1+infl,fraction);
    }
    const bridgeWhole=Math.ceil(bridgeYears);
    for(let y=0;y<bridgeWhole;y++){
      const fraction=(y===bridgeWhole-1 && bridgeYears%1)?bridgeYears%1:1;
      heloc*=1+helocRate*fraction;
      let need=spend*fraction;
      const fromBridge=Math.min(bridge,need);bridge-=fromBridge;need-=fromBridge;
      if(need>0){heloc+=need;if(heloc>helocLimit){bridgeOk=false;break}}
      const growth=Math.max(-.95,mean+vol*normalRandom());
      bridge=Math.max(0,bridge*(1+growth*fraction));
      restricted=Math.max(0,restricted*(1+growth*fraction));
      spend*=Math.pow(1+infl,fraction);
    }
    if(!bridgeOk)continue;
    bridgeSuccess++;helocUsedTotal+=heloc;
    let portfolio=Math.max(0,bridge+restricted-heloc),pension=separationPension,ok=true;
    for(let y=0;y<horizon;y++){
      const gap=Math.max(0,spend-pension);
      portfolio-=gap;
      if(portfolio<0){ok=false;break}
      portfolio=Math.max(0,portfolio*(1+Math.max(-.95,mean+vol*normalRandom())));
      spend*=1+infl;pension*=1+cola;
    }
    if(ok)lifetimeSuccess++;
  }
  return {bridge:Math.round(bridgeSuccess/runs*100),lifetime:Math.round(lifetimeSuccess/runs*100),avgHeloc:bridgeSuccess?helocUsedTotal/bridgeSuccess:0};
}
function readiness(d,now,start,target,currentPension,targetPension){const {investments}=netValues(d);const totalDays=Math.max(1,(target-start)/DAY),time=clamp(((Math.min(now,target)-start)/DAY)/totalDays*100);const pension=clamp(currentPension/Math.max(1,targetPension)*100);const investing=clamp(investments/Math.max(1,Number(d.investmentGoal))*100);const coverage=clamp(targetPension/Math.max(1,Number(d.annualBudget))*100);const debt=clamp((1-Number(d.mortgage)/Math.max(1,Number(d.mortgageOriginalInput)))*100);const liquid=Number(d.city457b)+Number(d.robinhoodTaxable)+Number(d.helocLimit);const buffer=clamp(liquid/(Number(d.annualBudget)*1.5)*100);const rows=[['Time readiness',time,25,'Toward eligibility'],['Pension progress',pension,25,'Current vs. eligibility pension'],['Investment progress',investing,20,'Toward investment goal'],['Spending coverage',coverage,15,'Pension vs. annual budget'],['Debt reduction',debt,10,'Original mortgage paid'],['Bridge buffer',buffer,5,'457(b) + taxable + HELOC capacity']];return{score:rows.reduce((s,r)=>s+r[1]*r[2]/100,0),rows}}
...
function render(){const d=load(),now=new Date(),start=localDate(d.careerStart),birth=localDate(d.birthDate),target=rule68Date(birth,start),separation=localDate(d.plannedSeparationDate||d.retirementDate),pensionStart=pensionStartForSeparation(separation,target),service=serviceYearsAt(start,now),targetYears=serviceYearsAt(start,target),separationYears=serviceYearsAt(start,separation),currentPension=pensionAt(d.pensionSalary,service),targetPension=pensionAt(d.pensionSalary,targetYears),separationPension=pensionAt(d.pensionSalary,separationYears),annualAccrual=Number(d.pensionSalary)*(service<15?.03:.035),daily=annualAccrual/YEAR_DAYS,totalDays=Math.max(1,(target-start)/DAY),served=Math.max(0,(Math.min(now,target)-start)/DAY),careerPct=clamp(served/totalDays*100),vals=netValues(d),ready=readiness(d,now,start,target,currentPension,targetPension),survival=survivabilityModel(d,separation,pensionStart,separationPension);
 d.retirementDate=target.toISOString().slice(0,10);save(d);
 todayLabel.textContent=now.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});
 const bridgeYears=Math.max(0,(pensionStart-separation)/DAY/YEAR_DAYS),fmt=d=>d.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});
 separationDateLabel.textContent=fmt(separation);pensionStartLabel.textContent=fmt(pensionStart);bridgeYearsLabel.textContent=bridgeYears<.01?'None':bridgeYears.toFixed(1)+' years';separationPensionLabel.textContent=money(separationPension)+'/yr';
 bridgeDetail.textContent=separation<target?`Leaving ${calendarCountdown(separation,target)} before Rule-of-68 eligibility delays pension commencement by the same amount beyond ${fmt(target)}. The bridge uses ${money(bridgeAccessibleTotal(d))} from the 457(b) and taxable account first, then up to ${money(d.helocLimit)} of undrawn HELOC capacity. The 175, Roth, and 403(b) accounts remain reserved for long-term retirement.`:`Pension is modeled to begin immediately at separation because the Rule-of-68 date has been reached.`;countdown.textContent=calendarCountdown(now,target);targetDateLabel.textContent=`Eligibility: ${target.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}`;progressBar.style.width=careerPct+'%';careerProgress.textContent=careerPct.toFixed(1)+'%';pensionToday.textContent=preciseMoney(currentPension);dailyAccrual.textContent='+'+preciseMoney(daily)+'/day';monthlyAccrual.textContent=`About ${money(daily*30.44)} per average month`;salaryBasis.textContent=`Annual benefit estimate using ${money(d.pensionSalary)} highest-year salary`;pensionAtTarget.textContent=money(targetPension);spendingCoverage.textContent=`Covers ${(targetPension/Number(d.annualBudget)*100).toFixed(0)}% of your ${money(d.annualBudget)} annual budget before taxes`;serviceYears.textContent=service.toFixed(6);serviceDetail.textContent=`Hired ${start.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}`;
 fireScore.textContent=Math.round(ready.score)+'%';setRing(fireRing,ready.score,'#56d39b');fireStatus.textContent=ready.score>=90?'Retirement-ready':ready.score>=75?'Strong progress':ready.score>=55?'Building momentum':'Foundation stage';fireChange.textContent=`Formula-based score across six readiness factors.`;bridgeScore.textContent=survival.bridge+'%';setRing(bridgeRing,survival.bridge,survival.bridge>=80?'#66a9ff':survival.bridge>=60?'#f5c66d':'#ff7d7d');bridgeStatus.textContent=survival.bridge>=90?'High bridge resilience':survival.bridge>=75?'Good bridge resilience':survival.bridge>=50?'Moderate bridge resilience':'Bridge at risk';bridgeScoreDetail.textContent=`Uses 457(b), taxable, then HELOC through ${fmt(pensionStart)}. Average HELOC drawn in successful runs: ${money(survival.avgHeloc)}.`;lifetimeScore.textContent=survival.lifetime+'%';setRing(lifetimeRing,survival.lifetime,survival.lifetime>=80?'#56d39b':survival.lifetime>=60?'#f5c66d':'#ff7d7d');lifetimeStatus.textContent=survival.lifetime>=90?'High modeled resilience':survival.lifetime>=75?'Good modeled resilience':survival.lifetime>=50?'Moderate modeled resilience':'Needs attention';lifetimeScoreDetail.textContent=`Includes the unpaid bridge, any HELOC balance, and ${money(separationPension)} pension accrued at separation. Planning estimate—not a guarantee.`;
...
function populateSettings(){const d=load(),start=localDate(d.careerStart),birth=localDate(d.birthDate),target=rule68Date(birth,start);d.retirementDate=target.toISOString().slice(0,10);settingIds.forEach(id=>{const el=document.getElementById(id);if(el)el.value=d[id]})}
settingsBtn.onclick=()=>{populateSettings();settingsDialog.showModal()};refreshBtn.onclick=()=>{buildBalanceFields();balancesDialog.showModal()};cancelBalances.onclick=()=>balancesDialog.close();snapshotBtn.onclick=()=>{const d=load();saveSnapshot(d,true);render()};balancesForm.onsubmit=e=>{e.preventDefault();const d=load();accountDefinitions.forEach(([id])=>d[id]=Number(document.getElementById(id).value||0));d.homeValue=Number(quickHomeValue.value||0);d.mortgage=Number(quickMortgage.value||0);d.balancesUpdatedAt=new Date().toISOString();saveSnapshot(d);save(d);balancesDialog.close();render()};settingsForm.onsubmit=e=>{e.preventDefault();const d=load();settingIds.forEach(id=>{const el=document.getElementById(id);d[id]=(el.type==='date'||el.type==='month')?el.value:Number(el.value||0)});d.retirementDate=rule68Date(localDate(d.birthDate),localDate(d.careerStart)).toISOString().slice(0,10);save(d);settingsDialog.close();render()};resetBtn.onclick=()=>{localStorage.removeItem(STORAGE_KEY);save(defaults);populateSettings();render()};
exportBtn.onclick=()=>{const blob=new Blob([JSON.stringify(load(),null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='retirement-hq-backup-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};
importFile.onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const parsed=JSON.parse(await file.text());save({...defaults,...parsed,snapshots:Array.isArray(parsed.snapshots)?parsed.snapshots:[]});populateSettings();render();alert('Backup imported successfully.')}catch{alert('That backup file could not be read.')}e.target.value=''};
window.addEventListener('resize',()=>drawChart(load().snapshots));render();setInterval(liveTick,1000);setInterval(render,300000);
